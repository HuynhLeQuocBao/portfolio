const AboutMe = () => {
    return (
        <div className=" flex flex-col justify-start items-start w-[1400px] h-[686.48px] pb-[0.48px] box-border margin">
            <div className=" flex flex-col justify-start items-start w-[100%] pb-[0.24px] box-border">
                <div className=" flex flex-row justify-between items-center w-[100%] h-[100%] pb-[0.24px] box-border">
                    <div className=" flex flex-col items-center h-[undefinedundefined] box-border max-w-[58em]">
                        <div className=" relative w-[100%] h-[292px] box-border">
                            <div className=" absolute flex flex-col w-[476.72px] h-[197px] box-border">
                                <p className="  border-[#000000ff] text-2xl leading-6  font-poppins  font-[600]  tracking-[0.72px]">
                                    Hi I am{" "}
                                </p>
                                <p className=" highlight-text  text-[32px] leading-8  font-poppins  font-[600]  tracking-[0.96px]">
                                    [Name]
                                </p>
                                <p className="  border-[#000000ff]  text-[100px]  leading-[120%]  font-poppins  font-[700]  tracking-[3px]">
                                    Software
                                </p>
                            </div>
                            <p className=" absolute top-[172px] left-[230px]  border-[#000000ff]  text-[100px]  leading-[120%]  font-poppins  font-[700]  tracking-[3px]">
                                Engineer{" "}
                            </p>
                        </div>
                        <p className=" flex flex-col justify-start text-start  border-[#000000ff]  text-[21px]  leading-[21px]  font-poppins  font-[400] mt-6 tracking-[0.63px]">
                            Lorem ipsum dolor sit amet consectetur. Tristique amet sed massa nibh lectus netus in. Aliquet donec morbi convallis pretium.
                            Turpis tempus pharetra
                        </p>
                    </div>
                    <div className="flex flex-col ">
                        <img
                            src="https://firebasestorage.googleapis.com/v0/b/unify-v3-copy.appspot.com/o/eopjsl8i9fb-1%3A236?alt=media&token=7b3a3aed-f77e-4bd1-9e28-b2c451e00e6b"
                            alt="Not Found"
                            className=" w-[538px] h-[100%]"
                        />
                        <div className="flex justify-center flex-row w-full mt-4">

                            <img
                                src="https://firebasestorage.googleapis.com/v0/b/unify-v3-copy.appspot.com/o/eopjsl8i9fb-1%3A242?alt=media&token=f2114610-e1ed-4aa1-ac3b-00d3c49994ea"
                                alt="Not Found"
                                className=" w-[205px] h-[32.48px]"
                            />
                        </div>
                    </div>

                </div>
            </div>


        </div>
    )
}
export default AboutMe; 